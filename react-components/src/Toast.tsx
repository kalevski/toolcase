...
export interface ToastOptions {
	/** Override the generated ID. Useful for deduplication. */
	id?: string
...
export interface ToastProviderProps {
	/** Auto-dismiss duration in ms for all toasts. 0 = persistent. Default: 4000. */
	defaultDuration?: number
	/** Default screen corner. Default: 'top-right'. */
	defaultPosition?: ToastPosition
	/** Maximum toasts visible per position group. Default: 5. */
	maxToasts?: number
...
class ToastStore {
	...
	add(message: string, variant: ToastVariant, options: ToastOptions = {}): string {
		...
		// Enforce maxToasts per position — drop the oldest in that group
		...
		this.items = [
			...this.items,
			{ id, message, variant, duration, position, title: options.title },
		]
		this.notify()
		return id
	}
...
const ToastItemInternal: React.FC<ToastItemInternalProps> = ({ item, fromBottom, onDismiss }) => {
	...
	// Mount: start auto-dismiss timer
	useEffect(() => {
		startTimer()
...
export const ToastProvider: React.FC<ToastProviderProps> = ({
	...
	// Sync config into the singleton store
	useEffect(() => {
