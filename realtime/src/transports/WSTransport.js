...
class WSTransport extends Transport {
    ...
    /** @private */
    logger = loggging.getLogger('transport:websocket')
    ...
    handleConnect = async (socket, request) => {
        ...
        if (ticket === null) {
            this.logger.verbose('client disconected, invalid ticket', ticket)
        ...
        try {
            ...
            this.logger.verbose(`grant access failure, ticket is not accepted, ticket=${ticket}, reason=${error.message}`)
            return socket.close(4001)
        }

        socket.on('message', (data, isBinary) => {
            if (!isBinary) {
                this.logger.verbose(`actor id=(${actorId}) ticket=(${ticket}) sent non-binary data`)
                socket.close(4002)
            }
