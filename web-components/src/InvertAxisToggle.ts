...
const TAG_NAME = 'tc-invert-axis-toggle'

// tc-invert-axis-toggle — an invert-axis on/off preset row. A pill-track switch
// (role="switch") paired with the shared setting-row scaffold. Port of
// game-components `gc-invert-axis-toggle` (which extends `gc-toggle-row`) with the
...
// `--bs-invert-axis-toggle-*`.
export class InvertAxisToggle extends SettingRowBase {
    ...
    attributeChangedCallback(name: string, old: string | null, next: string | null): void {
        ...
        if (name === 'checked') {
            const btn = this.querySelector<HTMLButtonElement>('.tc-invert-axis-toggle__switch')
    ...
    protected renderControl(): string {
        ...
        return `
            <button
                type="button"
                class="tc-invert-axis-toggle__switch"
                role="switch"
                aria-checked="${checked}"
                data-checked="${checked}"
                aria-label="${this.escape(this.rowLabel)}"${disabledAttr}
            >
                <span class="tc-invert-axis-toggle__knob"></span>
            </button>
        `
    }
