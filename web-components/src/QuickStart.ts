...
export class QuickStart extends HTMLElement {
    ...
    private render(): void {
        ...
        const titleHtml = titleText != null
            ? `<h2 class="tc-quick-start-title">${esc(titleText)}</h2>`
            : ''

        const stepsHtml = this._steps.length > 0
            ? `<ol class="tc-quick-start">${
                this._steps.map((step, i) => {
                    ...
                    const outputHtml = step.output != null
