...
export class GanttChart extends HTMLElement {
    ...
    private render(): void {
        ...
        const rowsHtml = resolved.map((r, i) => {
            ...
            const ariaName = esc(`${r.label}${groupLabel}: ${r.task.start} to ${r.task.end}${progressLabel}`)
            return `<div class="tc-gantt-chart-row">`
    ...
    private _onBarActivate = (e: Event): void => {
        ...
        this.dispatchEvent(new CustomEvent('tc-task-click', {
            bubbles: true,
            composed: true,
