...
export class Codex extends HTMLElement {
    ...
    private _detailMarkup(entry: CodexEntry): string {
        if (!entry.discovered) {
            return `<span class="tc-codex-detail-eyebrow">Codex</span>`
        ...
        const statsBlock = stats
            ? `<div class="tc-codex-detail-stats">${stats}</div>`
            : ''
        return `<span class="tc-codex-detail-eyebrow">Codex</span>`
    ...
    private render(): void {
        ...
        const rowsMarkup = this._entries.map(entry => {
            ...
            const glyph = discovered ? (lucideByName(entry.icon || DEFAULT_ICON) || lucideByName(DEFAULT_ICON)) : lockedIconHtml
            const name = discovered ? esc(entry.name) : '???'
            return `<div role="listitem" class="${classes.join(' ')}"`
        ...
        this.innerHTML = `<div class="tc-codex-list" role="list">${rowsMarkup}</div>`
            + `<div class="tc-codex-detail">${detailMarkup}</div>`

        if (focusedId != null) {
            this.querySelector<HTMLElement>(`.tc-codex-row[data-id="${CSS.escape(focusedId)}"]`)?.focus()
        }
    }
}
