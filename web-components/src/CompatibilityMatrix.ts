...
function statusIconHtml(status: CompatStatus): string {
    switch (status) {
        case 'yes':     return checkIconHtml
        case 'no':      return xIconHtml
        case 'partial': return minusIconHtml
        case 'unknown': return helpCircleIconHtml
    }
}
                ...
                const statusCells = platforms
                    .map(p => {
                        ...
                        const status: CompatStatus =
                            STATUSES.includes(raw as CompatStatus) ? (raw as CompatStatus) : 'unknown'
                        return (
