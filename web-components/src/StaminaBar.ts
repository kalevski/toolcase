...
const TAG_NAME = 'tc-stamina-bar'

export class StaminaBar extends HTMLElement {
    ...
    private render(): void {
        ...
        this.classList.add('tc-stamina-bar')
        ...
        const labelRow = label
            ? `<div class="tc-stamina-bar__label-row">` +
              `<span class="tc-stamina-bar__label">${escapeHtml(label)}</span>` +
              `<span class="tc-stamina-bar__label-value">${numericText}</span>` +
              `</div>`
            : ''

        const track = renderResourceBarTrack({
            prefix: 'tc-stamina-bar',
            value,
            max,
            ghost: this.ghost,
            ticks,
