...
const TAG_NAME = 'tc-mana-bar'

export class ManaBar extends HTMLElement {
    ...
    private render(): void {
        ...
        this.classList.add('tc-mana-bar')
        ...
        const labelRow = label
            ? `<div class="tc-mana-bar__label-row">` +
              `<span class="tc-mana-bar__label">${escapeHtml(label)}</span>` +
              `<span class="tc-mana-bar__label-value">${numericText}</span>` +
              `</div>`
            : ''
