...
export default class GameObjectPanel extends Panel {
    ...
    override draw(): void {
        ...
        this.components.alpha = this.base.addInput(this.gameObjectHandler, 'alpha', { label: 'Alpha', min: 0, max: 1 })
        this.components.coordsXY = this.base.addInput(this.gameObjectHandler, 'position', { label: 'Position', x: {}, y: {} })
        this.components.transformXY = this.base.addInput(this.gameObjectHandler, 'transform', { label: 'Transform', x: {}, y: {} })
        this.components.scaleXY = this.base.addInput(this.gameObjectHandler, 'scale', {
            label: 'Scale', x: { min: 0.1, max: 10 }, y: { min: 0.1, max: 10 }
        })
        this.components.angle = this.base.addInput(this.gameObjectHandler, 'angle', { label: 'Angle', min: 0, max: 360 })
