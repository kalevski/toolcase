...
// Flag is a feature flag's resolved value. Value is already coerced to its typed
// form server-side (bool | string | float64 | decoded JSON), matching Type, so the
// caller never type-asserts raw text (planning §6.2, gap-7).
type Flag struct {
...
// Client talks to a Wharf Agent API for one instance.
type Client struct {
	cfg  Config
	http *http.Client
...
func (c *Client) newRequest(ctx context.Context, path string) (*http.Request, error) {
	...
	req.Header.Set("X-Wharf-Instance", c.cfg.InstanceID)
	req.Header.Set("X-Wharf-Environment", c.cfg.Environment)
	return req, nil
}
...
func statusErr(res *http.Response) error {
	...
	return fmt.Errorf("wharf: %s -> %d: %s", res.Request.URL.Path, res.StatusCode, string(body))
}

// FetchEnv returns the resolved environment variables (secrets already resolved
// server-side for this instance).
...
// FetchFlags returns the resolved feature flags for this instance's environment.
...
// config changes (planning §6.2). Blocks until ctx is cancelled. The first
// successful fetch always fires cb.
