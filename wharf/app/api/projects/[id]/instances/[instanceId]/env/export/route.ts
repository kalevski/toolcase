...
import { exportInstance, ScopeNotFoundError, InterpolationCycleError, type ExportFormat } from '@/server/services/env-vars'
...
// Export the resolved instance env. Real secret values only for devops/owner;
// developers get masked placeholders. Value-bearing exports are audited.
export async function GET(req: Request, { params }: Ctx) {
    ...
    try {
        const text = exportInstance(id, instanceId, fmt, canReadSecrets)
        if (canReadSecrets) audit(auth, 'env.export', id, `instance:${instanceId}:${fmt}`)
