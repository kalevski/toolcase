// Pure secret-generation domain logic — planning §8.2. Given raw random bytes
// (supplied by the caller; this module performs NO I/O and does NOT import
// node:crypto) and a spec, produce a secret string deterministically. Buffer is
// used only as an encoding utility (hex / base64url). Keeping the entropy source
// out of here makes generation a pure function that is trivially testable with a
// fixed byte array.

import type { SecretGenKind } from '@/server/domain/types'

/** A request to generate one secret (planning §8.2). */
...
// Default character sets (planning §8.2).
...
/**
 * Deterministically map raw random `bytes` to a secret string per `spec`
 * (planning §8.2). The caller owns entropy; this function only encodes.
 *
