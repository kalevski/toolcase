// Authentication (spec §3): GitHub OAuth2 code flow + HMAC-signed httpOnly
// session cookie. All server-side. The GitHub access token is used only during
// the callback and never persisted. Ported from the blueprint, with voxscribe's
// role names (`admin | standard | guest`) and DB-backed role resolution.
...
export const SESSION_COOKIE = 'voxscribe_session'
export const STATE_COOKIE = 'voxscribe_oauth_state'
...
function isSecure(): boolean {
    // In production a cookie must always be `secure`, regardless of the redirect-
    // URI scheme: an http redirect URI in prod is a misconfiguration, not a reason
    // to ship session cookies over cleartext. Outside production, follow the
    // redirect-URI scheme so a plain-http dev loop still works.
...
/**
 * Nonces already redeemed by a callback. A signed `state` token stays valid for
 * its full 10-min TTL, so without this the same `state` + matching cookie could
 * be replayed within the window. Single-process app → an in-memory Map is
 * sufficient. Survives dev hot-reload via `globalThis`.
 */
declare global {
    var __voxscribeConsumedNonces: Map<string, number> | undefined
}

function consumedNonces(): Map<string, number> {
    if (!globalThis.__voxscribeConsumedNonces) globalThis.__voxscribeConsumedNonces = new Map()
    return globalThis.__voxscribeConsumedNonces
}
...
/**
 * Verify a `state` token's signature + expiry AND mark its nonce consumed
 * (single-use). The cookie pairing in the callback is the primary CSRF control;
 * this nonce set is best-effort defence-in-depth.
 */
...
export function buildAuthorizeUrl(state: string): string {
    // `read:user` identifies the user; an org gate additionally needs `read:org`.
    // voxscribe never touches repositories, so no `repo` scope.
...
// ── role policy (bootstrap admin / per-request re-read) ───────────────────────

/** Authoritative role lookup, re-read each call (§3). */
export function getRole(githubId: number): Role | null {
    return userRepo.get(githubId)?.role ?? null
}

/**
 * Resolve (or create) a user on login. The very first sign-in, when no admin
 * exists yet, bootstraps as `admin`; every subsequent new user lands as
 * `standard` (§3). Returning users keep their role and just have their profile
 * fields refreshed. Runs in a transaction so the `adminCount` check and the
...
export function resolveOnLogin(profile: GithubProfile): AppUser {
    return tx(() => {
        ...
        const role: Role = userRepo.adminCount() === 0 ? 'admin' : 'standard'
 ...
 * from SQLite (authoritative) so promotions/demotions take effect without a
 * re-login, falling back to `guest` for a session whose user row is gone (§3).
 */
