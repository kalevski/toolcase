...
export async function GET(req: NextRequest) {
    ...
    try {
        // The GitHub access token identifies the user here and is never persisted
        // (spec §3).
        ...
        try {
            auditRepo.append({ githubId: profile.githubId, login: profile.login, action: 'auth.login' })
        } catch {
            /* best-effort */
        }
