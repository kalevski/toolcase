...
- Read the task file at the path provided below and implement it end-to-end.
- Follow the repository's own conventions: read the repo-root CLAUDE.md and any knowledge/ docs first, and respect every hard rule you find there.
...
export class ExecutionManager extends EventEmitter {
    ...
    private async acquireLock(repo: string): Promise<void> {
        ...
        await fs.mkdir(repoTasksDir(repo), { recursive: true })
    ...
    private async runLoop(repo: string, opts: RunOptions): Promise<void> {
        ...
        try {
            ...
            for (const rel of discovered) {
                if (await this.passesStaticFilters(repo, rel, opts)) scoped.push(rel)
            }

            if (opts.reset) {
                for (const rel of scoped) {
                    await updateTaskStatus(repo, rel, 'open').catch(() => {})
                }
            }
            ...
            const preamble = (await readRepoPromptOverride(repo)) ?? DEFAULT_PREAMBLE
            ...
            while (i < scoped.length) {
                ...
                r.current = rel
                const absPath = path.join(repoTasksDir(repo), rel)
                ...
                if (opts.dryRun) {
                    this.log(
                        repo,
                        'comment',
                        `[dry-run] ${rel} → ${config.agentBin} --model ${resolveModel(opts.model)} (cwd=${repoPath(repo)})`,
                        rel,
                    )
                    i++
                    continue
                ...
                const outcome = await this.runOne(repo, rel, absPath, opts.model, preamble, warmSessionId)
    ...
    private async finalize(repo: string, reason: string): Promise<void> {
        const r = this.get(repo)
        r.child = null
        await clearWarmSession(repo).catch(() => {})
        await this.releaseLock(repo)
        await r.logger?.close()
    ...
    private async passesStaticFilters(repo: string, rel: string, opts: RunOptions): Promise<boolean> {
        ...
        if (opts.severity || opts.project) {
            const parsed = parseTask(await readTaskFile(repo, rel), rel)
            if (opts.severity) {
                const want = opts.severity.split(',').map((s) => s.trim().toLowerCase()).filter(Boolean)
                if (want.length && (!parsed.severity || !want.includes(parsed.severity))) return false
            }
            if (opts.project) {
                const want = opts.project.split(',').map((s) => s.trim().toLowerCase()).filter(Boolean)
