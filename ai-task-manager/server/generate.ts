...
import { listTaskFiles, repoTasksDir } from './fs-workspace'
...
async function highestPrefix(repo: string): Promise<number> {
...
export async function generateTasks(repo: string, userPrompt: string): Promise<GenerateResult> {
    const cwd = repoTasksDir(repo)
    await fs.mkdir(cwd, { recursive: true })

    const before = new Set(await listTaskFiles(repo))
    const next = (await highestPrefix(repo)) + 1
    const body = await skillBody()

    const prompt = [
        body,
        '',
        `The current highest task number is ${(await highestPrefix(repo))
            .toString()
    ...
    const after = await listTaskFiles(repo)
