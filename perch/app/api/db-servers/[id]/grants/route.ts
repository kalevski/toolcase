...
//     level ∈ none | read | readwrite | owner. Applying a level is a full
//     reset+grant, so it is idempotent and overwrites `custom` only explicitly (§3).
//
// Maintainer+ (perch_database_management.md §2, §8).
...
export async function PUT(req: Request, ctx: Ctx) {
    ...
    let body: { user?: unknown; database?: unknown; level?: unknown }
    try {
        body = (await req.json()) as { user?: unknown; database?: unknown; level?: unknown }
    } catch {
