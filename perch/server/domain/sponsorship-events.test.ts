...
describe('parseSponsorshipEvent', () => {
    ...
    it('falls back to null effectiveAt when the payload carries no timestamp', () => {
        const parsed = parseSponsorshipEvent({
            action: 'created',
            sponsorship: { sponsor: { login: 'bob' }, tier: { monthly_price_in_cents: 100 } },
        })
...
describe('reconcileToSponsorships', () => {
    ...
    it('maps active and cancelled nodes, skipping ones without a sponsor login', () => {
        const rows = reconcileToSponsorships(
            [
                {
                    sponsorEntity: { login: 'alice' },
                    tier: { monthlyPriceInCents: 2500 },
                    isActive: true,
                    createdAt: '2026-01-01T00:00:00Z',
                },
                { sponsorEntity: { login: 'bob' }, tier: { monthlyPriceInCents: 500 }, isActive: false },
        ...
        expect(rows).toEqual([
            ...
            { sponsorLogin: 'bob', tierCents: 500, status: 'cancelled', effectiveAt: NOW, updatedAt: NOW },
        ])
    })

    it('treats a missing tier price as 0 cents (→ free)', () => {
        const [row] = reconcileToSponsorships([{ sponsorEntity: { login: 'carol' }, isActive: true }], NOW)
