// Pure domain layer for access lists (perch_better.md C1) — named IP allow/deny
...
// hashes on GET (users carry `has_password` only), fragments Perch renders carry
...
export function parseAccessList(input: unknown): Check<AccessList> {
    ...
    for (const raw of Array.isArray(o.users) ? o.users : []) {
        ...
        // Plaintext or hashes never ride the fragment path from Perch.
