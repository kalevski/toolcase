// Scheduled resource-state poller (perch_better.md B1). NPM writes nginx errors onto
// the host row; Perch does strictly better — it PERSISTS per-resource state episodes
...
// daemon AND perch restarts — including the daemon's in-memory cert-job errors.
//
...
/**
 * Audit retention (B3): entries older than `PERCH_AUDIT_RETENTION_DAYS` days are
...
const AUDIT_RETENTION_DAYS = (() => {
    const raw = Number(process.env.PERCH_AUDIT_RETENTION_DAYS)
...
declare global {
    var __perchStatusPoll: { timer: ReturnType<typeof setInterval> } | undefined
}
...
async function collectBadStates(realmId: string): Promise<LiveBadState[]> {
    ...
    try {
        ...
        // /certs unavailable (older daemon / no cert dir) — nginx states still recorded.
    }
    return bad
}
...
export function ensureStatusPollStarted(): void {
    if (globalThis.__perchStatusPoll) return
    ...
    timer.unref?.()
    globalThis.__perchStatusPoll = { timer }
