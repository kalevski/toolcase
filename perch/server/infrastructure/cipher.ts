...
// secret Perch stores is a realm's nginxpilot admin token — a credential that grants
...
//
// Key source (§10.5): the dedicated `PERCH_REALM_KEY` when set (recommended), else a
// key HKDF-derived from `PERCH_AUTH_SECRET` so an existing deployment needs no new env.
// `PERCH_REALM_KEY_PREV` keeps a rotated-out key in the ring for decrypt.
//
...
/**
 * The realm-key secrets, newest first. When `PERCH_REALM_KEY` is set it (and any
 * `PREV`) are the ring; otherwise a single key is HKDF-derived from `PERCH_AUTH_SECRET`
...
function secrets(): string[] {
    ...
    const derived = crypto.hkdfSync(
        ...
        Buffer.alloc(0),
        Buffer.from('perch-realm-token-v1', 'utf8'),
        32,
    )
...
function ring(): RingEntry[] {
    ...
    if (out.length === 0)
        throw new CipherError(
            'no realm encryption key configured (PERCH_REALM_KEY / PERCH_AUTH_SECRET)',
        )
    cachedRing = out
    return out
}
...
function findKey(id: string): Buffer {
    ...
    if (!entry) {
        throw new CipherError(
            `no keyring entry for key id '${id}' — set PERCH_REALM_KEY_PREV to the key that sealed this data`,
        )
    }
    return entry.key
}
