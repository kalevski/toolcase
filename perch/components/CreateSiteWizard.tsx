...
const ERROR_COPY: Record<string, string> = {
    ...
    custom_domains_not_allowed: 'Custom domains require a paid (sponsor) plan. Use a Perch subdomain instead.',
    ...
    use_subdomain: "That domain is one of Perch's — attach it as a subdomain instead.",
...
export function CreateSiteWizard() {
    ...
    const hostKindOptions = useMemo<SingleCardSelectOption[]>(
        () => [
            {
                key: 'subdomain',
                title: 'Perch subdomain',
                description: 'Instant hosting on a Perch domain — no DNS setup. Free tier.',
            },
            {
                key: 'custom',
                title: 'Custom domain',
    ...
    async function submit() {
        ...
        if (hostKind === 'subdomain') {
            if (!baseDomain) return setError('No Perch base domain is available — use a custom domain instead.')
    ...
    if (created) {
        return (
            <div className="perch-wizard-success" role="status">
                <h2 className="perch-wizard-success-title">Site created</h2>
                <p className="perch-wizard-success-host">{created.hostname}</p>
                <p className="perch-wizard-success-body">
                    Your site is <strong>{created.status}</strong>. Perch is fetching the branch and
    ...
    return (
        <div className="perch-wizard">
            {error && (
                <div className="perch-wizard-error" role="alert">
                    {error}
                </div>
            )}
            ...
            <tc-form-wizard ref={wizardRef} complete-label="Create site" complete-icon="rocket">
                ...
                <div slot="step-1" className="perch-wizard-step">
                    ...
                    <div className="perch-wizard-host-mode" hidden={hostKind !== 'subdomain'}>
                        {baseDomains.length > 0 ? (
                            <>
                                <label className="perch-wizard-field-label">Subdomain</label>
                                <div className="perch-wizard-subdomain">
                                    <tc-input ref={labelRef} placeholder="alice" aria-label="Subdomain label" />
                                    <span className="perch-wizard-dot">.</span>
                                    <tc-extended-select
                                        ref={baseDomainRef}
                                ...
                                <p className="perch-wizard-preview">
                                    Your site will be served at <strong>{previewHost}</strong>
                                </p>
                            </>
                        ) : (
                            <p className="perch-wizard-hint">
                                No Perch subdomains are available yet. Use a custom domain below.
                            </p>
                        )}
                    </div>

                    <div className="perch-wizard-host-mode" hidden={hostKind !== 'custom'}>
                ...
                <div slot="step-2" className="perch-wizard-step">
                    ...
                    <p className="perch-wizard-hint">
                        Press <strong>Create site</strong> to provision it. Perch writes the deploy config and
