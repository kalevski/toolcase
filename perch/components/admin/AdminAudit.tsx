...
function auditRowsHtml(entries: AuditEntry[]): string {
    if (entries.length === 0) {
        return `<tr><td colspan="6" class="perch-admin-hint perch-audit-empty-row">No audit entries match.</td></tr>`
    }
    return entries
        .map((e) => {
            const snapshot = e.meta
                ? `<details class="perch-audit-snapshot"><summary>view</summary>` +
                  `<pre class="perch-admin-mono">${escapeHtml(prettyMeta(e.meta))}</pre></details>`
                : '<span class="perch-admin-hint">—</span>'
            return (
                `<tr>` +
                `<td class="perch-audit-time">${escapeHtml(fmtAt(e.at))}</td>` +
                `<td>@${escapeHtml(e.login ?? 'system')}</td>` +
                `<td><span class="perch-admin-mono">${escapeHtml(e.action)}</span></td>` +
                `<td>${e.site ? `<span class="perch-admin-mono">${escapeHtml(e.site)}</span>` : '<span class="perch-admin-hint">—</span>'}</td>` +
                `<td>${e.detail ? escapeHtml(e.detail) : '<span class="perch-admin-hint">—</span>'}</td>` +
                `<td>${snapshot}</td>` +
                `</tr>`
            )
        })
...
function AuditTable({ firstPage }: { firstPage: AuditPage }) {
    ...
    const tableProps = useMemo(
        ...
        [query.filters, query.order, query.offset, page.total, loading],
    )
    ...
    // Inject the body rows after the element rendered its <tbody> (relocation-safe:
    // React owns no slotted children).
    useEffect(() => {
        const el = tableRef.current
        if (!el) return
        const body = el.querySelector('.tc-advanced-table-body')
        if (body) body.innerHTML = auditRowsHtml(page.entries)
    }, [page.entries, tableRef])
