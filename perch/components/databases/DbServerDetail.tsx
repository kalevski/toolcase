...
// perch_database_management.md §9). Header + route-based SubTabBar (the
...
export function DbServerDetail({ serverId, tab }: { serverId: string; tab: DbServerTab }) {
    ...
    return (
        ...
        >
            {(server) => (
                <>
                    <button type="button" className="perch-back" onClick={() => router.push('/databases')}>
                        ← All servers
                    </button>
                    <p className="perch-admin-hint">
                        <span className="perch-admin-realm-name">{server.name}</span>{' '}
                        <span className="badge text-bg-info">{server.kind}</span>{' '}
                        <span className="perch-admin-mono">{`${server.host}:${server.port}`}</span>
                    </p>
                    <SubTabBar tabs={tabs} />
