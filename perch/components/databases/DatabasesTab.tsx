...
// Databases tab (perch_database_management.md §9): live table (name, owner,
            ...
            <tc-section-card title="Databases" icon="database">
                <div className="perch-admin-section">
                    ...
                    {databases.length === 0 ? (
                        ...
                        <tc-advanced-table
                            ref={setTableColumns}
                            limit={String(databases.length || 10)}
                            offset="0"
                            total={String(databases.length)}
                        >
                            {databases.map((db) => (
                                <tr key={db.name}>
                                    <td>
                                        <span className="perch-admin-mono">{db.name}</span>
                                    </td>
                                    <td>{db.owner ? <span className="perch-admin-hint">{db.owner}</span> : <span className="perch-admin-hint">—</span>}</td>
                                    ...
                                    <td className="text-end">
                                        <IconBtn icon="remove" label={`Drop ${db.name}`} danger disabled={busy} onClick={() => setDropping(db)} />
                                    </td>
                                </tr>
                            ))}
                        </tc-advanced-table>
