...
// re-point-to-global/secret preview, dotenv/JSON export, and a resolved-config
// preview panel (real secret values only for the owner role, §7).
...
const VAR_COLUMNS: TableColumn[] = [
    {
        key: 'key',
        header: 'Key',
        render: (row: VarRow) => `<span class="perch-admin-mono">${escapeHtml(row.key)}</span>`,
    },
    { key: 'source', header: 'Source', render: (row: VarRow) => badge(SOURCE_VARIANT[row.source], row.source) },
    {
        key: 'value',
        header: 'Value',
        render: (row: VarRow) => `<span class="perch-admin-mono perch-admin-hint">${escapeHtml(row.display)}</span>`,
    },
...
function VarsBody({ data, instance, onChanged }: { data: VarsData; instance: Instance; onChanged: () => void }) {
    ...
    return (
        <>
            <tc-section-card title="Environment variables" icon="variable">
                <div className="perch-admin-section">
                    ...
                    <div className="perch-instance-toolbar">
                        ...
                        <div className="perch-instance-toolbar-group" role="group" aria-label="Import and export">
            ...
            <ResolvedConfigPreview resolved={data.resolved} />
                ...
                >
                    ...
                    {!importRows ? (
                        ...
                        <FormGroup title={`Preview (${importRows.length} keys)`}>
                            ...
                            <div className="perch-form-import-rows">
                                {importRows.map((row, i) => (
                                    <div key={row.key} className="perch-form-item">
                                        <div className="perch-form-row">
                                            <span className="perch-admin-mono">{row.key}</span>
...
function ResolvedConfigPreview({ resolved }: { resolved: ResolvedConfig }) {
    return (
        <tc-section-card title="Resolved config preview" icon="eye">
            <div className="perch-admin-section">
                <p className="perch-home-lead perch-admin-hint">
                    What an instance fetch would resolve right now. Secret values are masked unless you’re the
                    owner; a real value is only ever served to a machine holding this instance’s fetch key.
                </p>
                {resolved.env.length === 0 ? (
                    <tc-empty-state icon="eye">Nothing resolves yet.</tc-empty-state>
                ) : (
                    <ul className="perch-resolved-list">
                        {resolved.env.map((e) => (
                            <li key={e.key} className="perch-resolved-row">
                                <span className="perch-resolved-key">{e.key}</span>
                                <span className="perch-resolved-value">{e.value}</span>
                                {e.pending && <span className="badge text-bg-warning">pending</span>}
                            </li>
                        ))}
                    </ul>
                )}
