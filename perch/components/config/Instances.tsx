...
export function Instances() {
    ...
    return (
        <ConfigPage
            title="Instances"
            subtitle="Flat list of instances, organized by tags. Maintainer access."
            icon="server"
            iconColor="cyan"
            state={state}
            onRetry={() => void reload()}
            ...
            <tc-section-card title="Instances" icon="server">
                <div className="perch-admin-section">
                    <p className="perch-home-lead perch-admin-hint">
                        {instances.length} instance{instances.length === 1 ? '' : 's'}. Tag instances to group them —
                        e.g. <code>api</code> + <code>production</code> instead of separate projects/environments.
                    </p>
                    ...
                    {rows.length === 0 ? (
                        <tc-empty-state icon="server">
                            {activeTags.length
                                ? `No instances tagged ${activeTags.map((t) => `“${t}”`).join(' + ')}.`
                                : 'No instances yet.'}
                        </tc-empty-state>
                    ) : (
                ...
                >
                    ...
                    <FormGroup title="Identity">
                        ...
                        <TextField
                            ...
                            help="Comma-separated. Replaces projects/environments — filter/group by tag."
                            value={form.tags}
