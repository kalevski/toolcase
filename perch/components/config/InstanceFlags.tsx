    ...
    return flags
        .map((f) => {
            ...
            return (
                ...
                `<td>${toggle}</td>` +
                `<td>${description}</td>` +
                `<td style="text-align:right"><span class="perch-admin-hint">${escapeHtml(fmtDate(f.updatedAt))}</span></td>` +
...
export function InstanceFlags({ instanceId }: { instanceId: string }) {
    ...
    const tableProps = useMemo(
        () => ({ columns: FLAG_COLUMNS, total: flags.length, limit: Math.max(flags.length, 1), offset: 0 }),
        [flags.length],
    )
    ...
    // whenever the flags change — including after a toggle/create/delete reload.
    useEffect(() => {
        const el = tableRef.current
        if (!el) return
        const body = el.querySelector('.tc-advanced-table-body')
        if (body) body.innerHTML = flagRowsHtml(flags)
    }, [flags, tableRef])
            ...
            <tc-section-card title="Feature flags" icon="flag">
                <div className="perch-admin-section">
                    ...
                    {flags.length === 0 ? (
                        ...
                        <tc-advanced-table ref={tableRef} className="perch-flags-table" />
                    )}
                </div>
            </tc-section-card>
