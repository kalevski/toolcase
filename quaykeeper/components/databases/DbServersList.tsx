...
// Maintainer entry page for database management (quaykeeper_database_management.md
...
// Body rows are fed to tc-advanced-table via its `rows` HTML string property
// (the canonical pattern — same as /admin/sites), so every interpolated value
// is escaped. The Manage control is a plain <button> with a data attribute;
// clicks are caught by delegation on the table host.
    ...
    return servers
        .map((s) => {
            ...
            return (
                ...
                `<td class="quaykeeper-admin-mono">${escapeHtml(`${s.host}:${s.port}`)}</td>` +
                `<td>${s.tls === 'require' ? `<span class="badge text-bg-light">require</span>` : `<span class="quaykeeper-admin-hint">off</span>`}</td>` +
                `<td>${status}</td>` +
...
function ServersTable({ servers }: { servers: DbServer[] }) {
    ...
    const tableProps = useMemo(
        () => ({
            ...
            limit: Math.max(servers.length, 1),
            offset: 0,
            rows: serversRowsHtml(servers),
        }),
        [servers],
    )
