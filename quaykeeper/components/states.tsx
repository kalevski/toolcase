...
export function LoadingState({
    ...
    return (
        <div className="quaykeeper-loading" role="status" aria-busy="true" aria-label={label}>
            {shape === 'text' && <tc-skeleton variant="text" count={count} />}

            {shape === 'cards' && (
                <div className="quaykeeper-card-grid">
                    {items.map((_, i) => (
                        <div className="quaykeeper-card quaykeeper-skeleton-card" key={i}>
            ...
            {shape === 'rows' && (
                <div className="quaykeeper-skeleton-rows">
                    {items.map((_, i) => (
            ...
            {shape === 'detail' && (
                <div className="quaykeeper-skeleton-detail">
...
export function EmptyState({
    ...
    return (
        <tc-empty-state icon={icon}>
            <strong className="quaykeeper-empty-title">{title}</strong>
            {description && <p className="quaykeeper-empty-desc">{description}</p>}
            {children}
        </tc-empty-state>
    )
}
