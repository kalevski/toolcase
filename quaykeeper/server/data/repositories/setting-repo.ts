// App-setting repository — all SQL for the `app_setting` table (§13, §12). A
// generic key/value store backing the owner-editable instance settings (branding +
// custom-domain ingress). Raw-SQL read-all / upsert only; validation and the
// default/env-fallback merge live in `domain/settings.ts` + `services/settings.ts`.
