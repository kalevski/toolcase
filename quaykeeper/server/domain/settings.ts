...
export interface SiteSettings {
    ...
    ingressIpv6: string
    /** Public base URL of this Quaykeeper instance (the UI/API origin, e.g. `https://quaykeeper.example.com`). */
    baseUrl: string
    /** Public URL of the instance-config (agent) server — the companion listener serving `/v1/*` (e.g. `https://config.example.com`). */
    instanceUrl: string
}
...
/** True for an absolute `http(s)://` URL with no query/fragment (a public origin + optional path). */
...
/** Normalize a stored URL: trim and drop trailing slashes so snippets can append paths. */
...
export function parseSettingsUpdate(input: unknown): SettingsCheck {
    ...
    if (src.baseUrl !== undefined) {
        if (typeof src.baseUrl !== 'string') {
            return { ok: false, reason: 'base_url', message: 'baseUrl must be a string' }
        }
        const v = normalizeUrl(src.baseUrl)
        if (v !== '' && !isHttpUrl(v)) {
            return { ok: false, reason: 'base_url', message: 'baseUrl must be an http(s) URL (or empty)' }
        }
        patch.baseUrl = v
    }

    if (src.instanceUrl !== undefined) {
        if (typeof src.instanceUrl !== 'string') {
            return { ok: false, reason: 'instance_url', message: 'instanceUrl must be a string' }
        }
        const v = normalizeUrl(src.instanceUrl)
        if (v !== '' && !isHttpUrl(v)) {
            return { ok: false, reason: 'instance_url', message: 'instanceUrl must be an http(s) URL (or empty)' }
        }
        patch.instanceUrl = v
    }
...
export function decodeStored(map: Record<string, string>): Partial<SiteSettings> {
    ...
    if (typeof baseUrl === 'string' && (baseUrl === '' || isHttpUrl(baseUrl))) out.baseUrl = baseUrl

    const instanceUrl = map[SETTING_KEYS.instanceUrl]
    if (typeof instanceUrl === 'string' && (instanceUrl === '' || isHttpUrl(instanceUrl))) out.instanceUrl = instanceUrl
