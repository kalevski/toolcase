// Pure dotenv parse + stringify (move_wharf_to_perch.md §4, §11). Ported
// verbatim from wharf's `server/domain/env-file.ts`: the shared serializer
// behind the .env import preview and export — no I/O, so it round-trips
// deterministically and is unit-testable without touching the filesystem. The
// caller dedupes the ordered entries that `parse` returns.
...
/**
 * Parse a .env document into ordered entries. Handles:
...
/** Decode the raw RHS of an assignment per the quoting rules above. */
...
/**
 * Serialize entries to a .env document: one `KEY=value` per line plus a
 * trailing newline. A value is double-quoted (escaping \ " and newline) when
 * it is empty OR contains whitespace, `#`, `=`, a quote, or a newline;
