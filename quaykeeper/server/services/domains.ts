    ...
    for (const { domain: base } of baseDomainRepo.listByRealm(realmId)) {
        ...
        if (host === b || host.endsWith(`.${b}`)) {
            throw new HostnameError(
                `"${host}" is under a Quaykeeper base domain — attach it as a subdomain instead`,
                'use_subdomain',
                400,
            )
        }
 ...
 * else the `QUAYKEEPER_INGRESS_IPV4` env default). **Fails closed** at every step: an
...
export async function verifyCustomDomain(domain: string): Promise<DomainVerification> {
    ...
    if (!expected) {
        ...
        // knows to set the ingress IP (admin Settings, or QUAYKEEPER_INGRESS_IPV4) (§16).
        throw new HostnameError(
