 ...
 *
 *   • owner    → the signed `quaykeeper_realm` cookie's realm if it still exists, else the
 ...
 * trailing `/`, blocks the cloud-metadata host, and — when `QUAYKEEPER_REALM_URL_ALLOWLIST` is
...
export interface RealmTestResult {
    ...
    disabled: number
    /** The daemon's self-reported API version (`GET /schema` info.version, impl §6); absent when the probe failed. */
    apiVersion?: string
...
export async function testRealm(id: string): Promise<RealmTestResult> {
    ...
    // Daemon API version — best-effort (impl §6): a transient /schema failure is
...
/**
 * The active realm's daemon capabilities, or `null` when `GET /schema` is unreachable —
 * callers MUST treat `null` as "unknown, assume everything", never as "no capabilities".
 */
 ...
 * table is empty, register the env-config instance (`QUAYKEEPER_NGINXPILOT_ADMIN_URL`/`TOKEN`) as
 * the single default realm. Safe to call repeatedly.
 */
