...
Server-side layering for Quaykeeper (mirrors TaskForge). Request flow, top to bottom:
...
- `config.ts` is the single, validated read of `process.env` (fail-fast on a
  ...
  `QUAYKEEPER_AUTH_SECRET` cookie key, and the session TTL all live here.
...
- `infrastructure/` holds server-only adapters to external systems (no DB, no
  ...
  access token (read from the `quaykeeper_gh_token` `httpOnly` cookie set at login).
  ...
  `infrastructure/server-log.ts` is the `[quaykeeper]` structured stdout logger.
  `infrastructure/nginxpilot.ts` is the deploy-engine seam (§4), driven ENTIRELY over
  nginxpilot's admin REST API — Quaykeeper never touches the daemon's filesystem, so the two
...
- `services/domains.ts` owns the shared **hostname namespace** + custom-domain
  ...
  fails closed unless it points at `QUAYKEEPER_INGRESS_IPV4` (takeover guard); and
