// nginxpilot integration client — the load-bearing seam Quaykeeper drives the deploy
// engine through (§4). Quaykeeper never touches nginxpilot's filesystem: the entire
...
//
// Multiple-realms (multiple_realms.md): every call goes through an explicit
// {@link RealmConnection} via {@link nginxpilotClient} — callers always resolve
// a per-realm client first.
//
...
export interface NginxpilotSiteStatus {
     ...
     * §11/§17 cross-repo field that lets Quaykeeper enforce byte quotas without shared-FS
     * access). Absent before the first successful sync.
     */
    bytes?: number
}
 ...
 * else) — the same keying Quaykeeper uses to match a resource back to one of its entities.
 */
 ...
 * real_ip`) — Quaykeeper surfaces it read-only.
 */
 ...
 * acme is enabled, so a control plane can tell "disabled" from "status unavailable".
 ...
 * `QUAYKEEPER_NGINXPILOT_TIMEOUT_MS`. An abort surfaces as a `NginxpilotError` (→ 502).
 */
const ADMIN_FETCH_TIMEOUT_MS = (() => {
    const raw = Number(process.env.QUAYKEEPER_NGINXPILOT_TIMEOUT_MS)
 ...
 * own ceiling is ~180s. Quaykeeper must wait LONGER than the daemon, or it aborts the request
 * mid-run and reports a detail-less `nginxpilot_error` while certbot is still working.
 * Default 300s; override via `QUAYKEEPER_NGINXPILOT_CERT_TIMEOUT_MS`.
 */
const CERT_OP_TIMEOUT_MS = (() => {
    const raw = Number(process.env.QUAYKEEPER_NGINXPILOT_CERT_TIMEOUT_MS)
...
export function nginxpilotClient(conn: RealmConnection) {
     ...
     * from (`<domain>.yml`), not Quaykeeper's internal site id.
     */
     ...
     * exists". Throws (via adminOk) when the endpoint is unreachable — callers treat
     * that as "capabilities unknown", never as "no capabilities".
     */
...
// Every caller resolves a per-site / per-realm / active-realm client through
// `services/realms.ts`. The env config is consumed only by `realms.ensureSeed`,
// which registers it as the default realm.
