...
// `infrastructure/server-log.ts`, rebranded `[quaykeeper]`.

import 'server-only'

type Level = 'info' | 'warn' | 'error'

/** Structured one-line server log: `[quaykeeper] <ISO> <LEVEL> <scope> — <message> {…}`. */
export function slog(level: Level, scope: string, message: string, extra?: Record<string, unknown>): void {
    const line = `[quaykeeper] ${new Date().toISOString()} ${level.toUpperCase()} ${scope} — ${message}`
