// MySQL / MariaDB driver (quaykeeper_database_management.md §6). Same shape as the
...
import type { DbDatabase, DbGrant, DbOperation, DbUser } from '@/server/domain/types'
import { quoteAccount, quoteIdent, quoteLiteral } from '@/server/domain/db-identifiers'
import { classifyAccess, grantPlan, operationGrantPlan, type GrantPlan } from '@/server/domain/db-access'
import {
    CONNECT_TIMEOUT_MS,
...
export const mysqlDriver: DbDriver = {
    ...
    async applyAccess(conn, user, database, level) {
        await runPlan(conn, user, (host) => grantPlan('mysql', user, database, level, host))
    },

    async applyOperations(conn, user, database, operations: readonly DbOperation[]) {
        await runPlan(conn, user, (host) => operationGrantPlan('mysql', user, database, operations, host))
    },
}

/** Resolve the account host, build the plan, and run it — tolerating the
 *  expected "no such grant" from the reset REVOKE (§6). */
async function runPlan(
    conn: DbConnInfo,
    user: string,
    planFor: (host: string) => GrantPlan,
): Promise<void> {
    await withConn(conn, async (c) => {
