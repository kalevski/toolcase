...
//
// Both render a plain `<button class="btn btn-sm btn-outline-… quaykeeper-icon-btn">`
...
// width/height; strip them (like web-components' icon()) so `.quaykeeper-icon-btn`
// CSS owns sizing.
...
export function iconBtnHtml({ icon, label, data, danger, disabled }: IconBtnHtmlOptions): string {
    const attrs = Object.entries(data)
        ...
        .join('')
    return (
        `<button type="button" class="btn btn-sm btn-outline-${danger ? 'danger' : 'secondary'} quaykeeper-icon-btn"` +
...
export function IconBtn({
    ...
    return (
        <button
            type="button"
            className={`btn btn-sm btn-outline-${danger ? 'danger' : 'secondary'} quaykeeper-icon-btn`}
            title={label}
            aria-label={label}
            disabled={disabled}
            onClick={onClick}
