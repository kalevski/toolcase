const TAG_NAME = 'gc-objective-marker'
...
export class ObjectiveMarker extends HTMLElement {
    ...
    private render(): void {
        ...
        if (size != null) this.style.setProperty('--gc-objective-marker-size', `${size}px`)
        else this.style.removeProperty('--gc-objective-marker-size')
        if (color) this.style.setProperty('--gc-objective-marker-color', color)
        else this.style.removeProperty('--gc-objective-marker-color')

        const label = this.label
        const distance = this.distance
        const distanceMarkup = distance != null
            ? `<span class="gc-objective-marker-distance">${this.formatDistance(distance)}</span>`
            : ''
        const labelMarkup = label
            ? `<span class="gc-objective-marker-label">${this.escape(label)}</span>`
            : ''

        this.innerHTML = `
            <div class="gc-objective-marker-glyph">◆</div>
            <div class="gc-objective-marker-text">
                ${labelMarkup}
                ${distanceMarkup}
            </div>
        `
...
if (typeof customElements !== 'undefined' && !customElements.get(TAG_NAME)) {
    customElements.define(TAG_NAME, ObjectiveMarker)
}
