...
Each component reads its colors and dimensions from CSS custom properties scoped to its root. Override them locally for one-off tweaks, or globally inside a theme. Pattern: `--{abbr}-{semantic}` where `semantic` is `color`, `muted`, `border`, `bg`, `hover-bg`, `active-color`, `active-border`, `transition`, `shadow` (no `radius` — square corners are a design rule).

| Component(s) | Prefix | Common knobs |
|---|---|---|
| Library globals | `--tc-` | shared color tokens consumed by themes |
| Bootstrap layer | `--bs-` | Bootstrap 5 design tokens (always present) |
| AdvancedTable | `--at-` | `bg`, `border`, `text`, `text-muted`, `accent`, `shadow`, `transition` |
| Dropdown | `--rc-dropdown-` | `border`, `color`, `muted`, `hover-bg`, `active-border`, `active-color`, `transition` |
| Pagination | `--pg-` | `color`, `muted`, `border`, `active-color`, `active-border` |
| SideNav | `--side-nav-` | `color`, `muted`, `border`, `active-color`, `active-border` |
| FormInput | `--fi-` | `color`, `muted`, `border`, `error-border`, `focus-border` |
| ExtendedSelect | `--es-` | `border`, `bg`, `hover-bg`, `active-color` |
| NumberInput | `--ni-` | `color`, `bg`, `border`, `focus-border`, `error-border`, `hover-border`, `btn-bg`, `btn-hover-bg`, `height`, `transition` |
| AnnouncementBar | `--ab-` | `bg`, `text`, `border`, `accent`, `subtle-bg`, `muted` |
...
| `size` | `'small' \| 'default' \| 'large'` | ❌ | Visual size of the input + label. Default: `'default'`. |
| `className` | `string` | ❌ | Class on the wrapper |
| `inputClassName` | `string` | ❌ | Class on the `<input>` element |

Also accepts all `HTMLInputElement` attributes except the native `size` (`checked`, `onChange`, `disabled`, etc.).
...
Also accepts all `HTMLInputElement` attributes except the native `size`.
...
| `label` | `string` | ❌ | Field label |
| `error` | `string` | ❌ | Validation error message |
| `size` | `'small' \| 'default' \| 'large'` | ❌ | Visual size — maps to Bootstrap `form-select-sm` / `form-select-lg`. Default: `'default'`. |
| `className` | `string` | ❌ | Class on the wrapper |
| `selectClassName` | `string` | ❌ | Class on the `<select>` element |

Also accepts all `HTMLSelectElement` attributes except the native `size`.
...
**Controlled vs uncontrolled** — Select forwards every prop to the native `<select>`. Pass `value` + `onChange` for a controlled select, or `defaultValue` (and read `event.currentTarget.value` from `onChange` when needed) for an uncontrolled one. Don't pass both — React will warn.
...
**Controlled vs uncontrolled** — props pass through to the native `<textarea>`. Use `value` + `onChange` for a controlled textarea, or `defaultValue` for an uncontrolled one and read the value from a ref or via `onChange={(e) => …}`. Mixing the two will trigger React's controlled/uncontrolled warning.
...
A loading placeholder with configurable variant and dimensions.

| Prop | Type | Required | Description |
|------|------|----------|-------------|
| `variant` | `'text' \| 'circle' \| 'rect'` | ❌ | Variant (default: `'text'`) |
...
**Usage notes** — Tag is presentational, so there is no internal state to control. The host owns the underlying tag list; the `onRemove` callback fires only when `removable` is true and the user clicks the × button. To render a single tag with no remove affordance, omit both props.
...
| Key | Action |
|-----|--------|
| `Tab` / `Shift+Tab` | Move focus between section headers |
...
| `size` | `'small' \| 'default' \| 'large'` | ❌ | Visual size — adjusts control height + font sizes. Default: `'default'`. |
| `className` | `string` | ❌ | Additional CSS class on the root |

Also accepts all `HTMLInputElement` attributes except `type`, `value`, `onChange`, and the native `size`.
...
**Keyboard** (focus a node first):

| Key | Action |
|-----|--------|
| `Tab` / `Shift+Tab` | Move focus to next/previous node |
| `Enter` / `Space` | Toggle selection of focused node |
| `→` | Expand the focused branch (when collapsed) |
| `←` | Collapse the focused branch (when expanded) |
| `Enter` (on chevron) | Expand or collapse without changing selection |
...
**Keyboard** (carousel root must be focused):
...
Hovering or focusing the carousel pauses auto-play; it resumes on blur.
