...
class AudioPanelDemo extends Scene {
    ...
    onInit() {
        ...
        this.busSfx = audio.addBus('sfx', (bus) => {
            console.log('[sfx bus]', bus.volume, 'mute=', bus.mute)
        })
        this.busMusic = audio.addBus('music', (bus) => {
            console.log('[music bus]', bus.volume, 'mute=', bus.mute)
        })
    }
    ...
    onCreate() {
        ...
        this.add.text(width / 2, 100,
            'Adjust Master vol/mute and per-bus sliders. Open console for change events.', {
                font: '14px Courier',
                color: '#aaa'
            }).setOrigin(0.5)
