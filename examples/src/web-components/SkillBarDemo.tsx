...
const SkillBarDemo: React.FC = () => {
    ...
    useEffect(() => {
        ...
        stateRef.current.slots = [
            { id: 'shield', name: 'Shield Wall', icon: 'Shield', hotkey: 'Q', cooldown: 12, selected: true },
            { id: 'stun', name: 'Stun', icon: 'Swords', hotkey: 'W', cooldown: 6, charges: 2 },
            { id: 'dash', name: 'Dash', icon: 'Wind', hotkey: 'E', cooldown: 8, disabled: true },
            { id: 'ult', name: 'Supernova', icon: 'Star', hotkey: 'R', cooldown: 90, disabled: true },
        ]
    }, [])
    ...
    useEffect(() => {
        ...
        cdRef.current.slots = [
            { id: 'slash', name: 'Quick Slash', icon: 'Sword', hotkey: 'Q', cooldown: 6, remaining: 5 },
            { id: 'heal', name: 'Mend', icon: 'Heart', hotkey: 'W', cooldown: 20, remaining: 7 },
            { id: 'charge', name: 'Charge', icon: 'ChevronsRight', hotkey: 'E', cooldown: 18, remaining: 0 },
            { id: 'ult', name: 'Obliterate', icon: 'Skull', hotkey: 'R', cooldown: 90, remaining: 0 },
        ]
    }, [])
                ...
                <div className="row">
                    <div className="col-12">
                        ...
                        <div className="d-flex flex-column gap-4 mt-4">
                            <SectionCard title="Basic — four abilities with hotkeys and cooldowns">
                                {/* @ts-ignore */}
                                <tc-skill-bar ref={basicRef} />
                            </SectionCard>

                            <SectionCard title="States — selected, charges, and disabled">
                                {/* @ts-ignore */}
                                <tc-skill-bar ref={stateRef} />
                            </SectionCard>

                            <SectionCard title="Active cooldown overlays">
                                {/* @ts-ignore */}
                                <tc-skill-bar ref={cdRef} />
                            </SectionCard>

                            <SectionCard title="Events — tc-activate">
                                {/* @ts-ignore */}
                                <tc-skill-bar ref={eventsRef} />
                                <div className="mt-3">
                                    <strong className="d-block mb-1">Event log</strong>
                                    {log.length === 0 ? (
                                        <span className="text-muted">Click a skill card to fire tc-activate…</span>
                                    ) : (
                                        <ul className="mb-0">
                                            {log.map((line, i) => (
                            ...
                            <SectionCard title="Custom slot-size (220px) and gap (1rem)">
                                {/* @ts-ignore */}
                                <tc-skill-bar
                                    ref={(el: any) => {
                                        if (!el) return
                                        el.slots = [
                                            { id: 'bolt', name: 'Chain Lightning', icon: 'Zap', hotkey: 'Q', cooldown: 8 },
                                            { id: 'smite', name: 'Holy Smite', icon: 'Sun', hotkey: 'W', cooldown: 14 },
                                        ]
                                    }}
                                    slot-size="220"
                                    gap="1rem"
