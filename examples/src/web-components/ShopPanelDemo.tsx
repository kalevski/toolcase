...
const ShopPanelDemo: React.FC = () => {
    ...
    useEffect(() => {
        ...
        buyRef.current.items = [
            ...
            { item: { id: 'dragon-shield', name: 'Dragon Shield', icon: '◈' }, price: 999, soldOut: true },
        ]
    }, [])
    ...
    useEffect(() => {
        ...
        discountRef.current.items = [
            { item: { id: 'elixir', name: 'Elixir of Power', icon: '✦' }, price: 100, discount: 0.3 },
            { item: { id: 'cloak', name: 'Cloak of Shadows', icon: '◆' }, price: 400, discount: 0.5 },
    ...
    useEffect(() => {
        ...
        const onBuy = (e: CustomEvent) => setLog(l => [`tc-buy — id: "${e.detail.id}"`, ...l].slice(0, 8))
                ...
                <div className="row">
                    <div className="col-12">
                        ...
                        <div className="d-flex flex-column gap-4 mt-4">
                            <SectionCard title="Buy mode — currency balance and affordability states">
                                {/* @ts-ignore */}
                                <tc-shop-panel ref={buyRef} currency-icon="◆" style={{ maxWidth: '440px' }} />
                            </SectionCard>

                            <SectionCard title="Sell mode — sell-mode attribute">
                                {/* @ts-ignore */}
                                <tc-shop-panel ref={sellRef} sell-mode style={{ maxWidth: '440px' }} />
                            </SectionCard>

                            <SectionCard title="Discounted items — strikethrough old price and discount badge">
                                {/* @ts-ignore */}
                                <tc-shop-panel ref={discountRef} currency-icon="$" style={{ maxWidth: '440px' }} />
                            </SectionCard>

                            <SectionCard title="Empty state — no items">
                                {/* @ts-ignore */}
                                <tc-shop-panel style={{ maxWidth: '440px' }} />
                            </SectionCard>

                            <SectionCard title="Events — tc-buy">
                                {/* @ts-ignore */}
                                <tc-shop-panel ref={eventsRef} currency-icon="◆" style={{ maxWidth: '440px' }} />
                                <div className="mt-3">
                                    <strong className="d-block mb-1">Event log</strong>
                                    {log.length === 0 ? (
                                        <span className="text-muted">Click Buy to fire tc-buy…</span>
                                    ) : (
                                        <ul className="mb-0">
                                            {log.map((line, i) => (
