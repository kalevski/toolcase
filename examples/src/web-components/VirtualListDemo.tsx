...
const FixedDemo: React.FC = () => {
    ...
    return (
        <tc-section-card title="10,000 rows — fixed height, lazy loading on scroll-to-end">
            ...
            >
                {count.toLocaleString()} rows loaded — only the visible window lives in the DOM.
                Scroll to the bottom to append 5,000 more.
            </p>
        </tc-section-card>
    )
}
    ...
    const ref = useTc<HTMLElement>({
        ...
        renderItem: (item: Row, index: number) => {
            ...
            const body = Array.from({ length: lines })
                .map(
                    () =>
                        `<div style="color:var(--tc-text-muted);font-size:0.75rem">Detail line for ${item.name}</div>`,
                )
                .join('')
            wrap.innerHTML =
    ...
    return (
        <tc-section-card title="Variable row heights (2,000 rows)">
            ...
            <p
                ...
                style={{ fontSize: '0.8125rem', color: 'var(--tc-text-muted)' }}
            >
                <code>itemHeight</code> is a function returning a different height per index; a
                cumulative offset table keeps scrolling accurate.
            </p>
        </tc-section-card>
    )
}
