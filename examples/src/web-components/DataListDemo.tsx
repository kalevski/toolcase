...
const ACHIEVEMENTS = [
    { id: 'first-blood', name: 'First Blood', description: 'Defeat your first enemy.', unlocked: true, points: 10 },
    { id: 'explorer', name: 'Explorer', description: 'Discover 25 hidden locations.', progress: 18, target: 25, points: 50 },
    { id: 'collector', name: 'Collector', description: 'Gather every rare artifact.', progress: 7, target: 40, points: 100 },
    { id: 'untouchable', name: 'Untouchable', description: 'Finish a boss without taking damage.', unlocked: false, points: 75 },
]
...
const renderCredit = (s: any) =>
    ...
    (s.names as string[]).map(n => `<span class="tc-data-list__primary">${esc(n)}</span>`).join('') +
    `</div>` +
    `</li>`
...
const renderAchievement = (a: any) => {
    ...
    return (
        ...
        (a.description ? `<span class="tc-data-list__secondary">${esc(a.description)}</span>` : '') +
        progress +
        `</div>` +
        (typeof a.points === 'number' ? `<span class="tc-data-list__trailing">${a.points} pts</span>` : '') +
        `</li>`
    )
}
...
const renderSlot = (s: any) => {
    ...
    const actions = s.empty
        ...
        : `<button type="button" class="tc-data-list__action" data-action="load">Load</button>` +
          (s.autosave ? '' : `<button type="button" class="tc-data-list__action" data-action="delete">Delete</button>`)
    return (
...
const DataListDemo: React.FC = () => {
    ...
    useEffect(() => {
        if (muteRef.current) {
            ...
            const handler = (e: any) =>
                setLog(l => [`tc-action — action: "${e.detail.action}", id: "${e.detail.id}"`, ...l].slice(0, 8))
    ...
    useEffect(() => {
        ...
        const onAction = (e: any) =>
            setSlotLog(l => [`tc-action — "${e.detail.action}" id="${e.detail.id}"`, ...l].slice(0, 8))
                ...
                <div className="row">
                    <div className="col-12">
                        ...
                        <div className="d-flex flex-column gap-4 mt-4">
                            <SectionCard title="Muted players — per-row action (tc-action)">
                                {/* @ts-ignore */}
                                <tc-data-list ref={muteRef} empty-text="No muted players." style={{ maxWidth: '480px' }} />
                                <div className="mt-3">
                            ...
                            <SectionCard title="Team members — avatars, emails, role chips">
                                {/* @ts-ignore */}
                                <tc-data-list ref={teamRef} style={{ maxWidth: '480px' }} />
                            </SectionCard>

                            <SectionCard title="Credits — grouped sections via renderRow">
                                {/* @ts-ignore */}
                                <tc-data-list ref={creditsRef} list-title="Credits" style={{ maxWidth: '480px' }} />
                            </SectionCard>

                            <SectionCard title="Achievements — progress bars + points trailing">
                                {/* @ts-ignore */}
                                <tc-data-list ref={achRef} style={{ maxWidth: '560px' }} />
                            </SectionCard>

                            <SectionCard title="Friends roster — status pips, count header, sorted, two actions">
                                {/* @ts-ignore */}
                                <tc-data-list ref={friendsRef} style={{ maxWidth: '440px' }} />
                            </SectionCard>

                            <SectionCard title="Save slots — selectable listbox + per-row actions (tc-select / tc-action)">
                                {/* @ts-ignore */}
                                <tc-data-list ref={slotsRef} selectable selected-id="s1" style={{ maxWidth: '560px' }} />
                                <div className="mt-3">
                                    <strong className="d-block mb-1">Event log</strong>
                                    {slotLog.length === 0 ? (
                                        <span className="text-muted">Select a slot or click an action…</span>
                                    ) : (
                            ...
                            <SectionCard title="Empty state (empty-text)">
                                {/* @ts-ignore */}
                                <tc-data-list ref={emptyRef} empty-text="Nothing to show yet." style={{ maxWidth: '480px' }} />
                            </SectionCard>
                        </div>
                    </div>
                </div>
