...
type sendError struct {
	err        error
	permanent  bool          // 4xx-style: retrying can never succeed — drop the batch
	retryAfter time.Duration // server-provided backoff hint (429/503 Retry-After)
}

func (e *sendError) Error() string { return e.err.Error() }
func (e *sendError) Unwrap() error { return e.err }

// newSink builds the sink for a destination.
...
// writerSink writes NDJSON lines to an io.Writer (stdout destination — pipes
// access logs into the container's own log stream).
...
// fileSink appends NDJSON to a local file with simple size-based self-rotation
// (path → path.1 → path.2 …, oldest removed) — the "I don't run Loki yet"
// destination, and the only local-disk persistence path (G4: no per-resource
// access-log files to rotate).
...
// rotate shifts path.(n) → path.(n+1), dropping the oldest, then reopens.
