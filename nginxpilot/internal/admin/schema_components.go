...
func componentSchemas() map[string]any {
	...
	return map[string]any{
		...
		"LogDestination": obj(map[string]any{
			"name":                 str(),
			"type":                 strEnum("loki", "http", "file", "stdout"),
			"enabled":              boolean(),
			"url":                  str(),
			"tenant":               str(),
			...
			"auth": obj(map[string]any{
				"method":        strEnum("none", "basic", "bearer"),
				"username":      str(),
				"password_env":  str(),
				"password_file": str(),
				"token_env":     str(),
			...
			"filter":         map[string]any{"type": "object", "additionalProperties": arr(str())},
			"sample":         map[string]any{"type": "number"},
			"batch_size":     integer(),
			"flush_interval": str(),
			"max_retries":    integer(),
			"buffer_size":    integer(),
		...
		"LogDestinationList": obj(map[string]any{"log_destinations": arr(ref("LogDestination"))}),
		"LogDestStatus": obj(map[string]any{
			"name":            str(),
			"type":            str(),
			"shipped":         integer(),
			"dropped":         integer(),
		...
		"LogsStatus": obj(map[string]any{
			"enabled":       boolean(),
			"syslog_listen": str(),
			"intake_error":  str(),
			"received":      integer(),
			"parse_errors":  integer(),
			"destinations":  arr(ref("LogDestStatus")),
		}, "enabled", "destinations"),
		"LogDestTestResult": obj(map[string]any{
			"ok":    boolean(),
			"name":  str(),
			"error": str(),
		}, "ok", "name"),
