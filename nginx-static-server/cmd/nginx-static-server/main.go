// Command nginx-static-server is a daemon that keeps directories of static
...
const usage = `nginx-static-server — keep nginx static site directories in sync with remote sources

Usage:
  nginx-static-server [run] [flags]         run the daemon (default)
  nginx-static-server validate [flags]      parse + validate the merged config
  nginx-static-server sync <domain> [flags] one-shot sync for one site
  nginx-static-server print-vhost <domain>  print an nginx server-block snippet
  nginx-static-server status [--json]       show per-site status from the daemon
  nginx-static-server version               print build info

Common flags:
  --config PATH        config file (default /etc/nginx-static-server/config.yml)
...
func main() {
	...
	case "status":
		code = cmdStatus(args)
	case "version":
		fmt.Printf("nginx-static-server %s\n", version)
	case "help", "--help", "-h":
		fmt.Print(usage)
	default:
